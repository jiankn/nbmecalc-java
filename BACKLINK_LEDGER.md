...
- Pending / prepared: 3 new platform surfaces (`r-universe.dev`, `packagecontrol.io`, `glama.ai`)
...
| DEV Community | dev.to | https://dev.to/jiankn/building-an-uncertainty-aware-practice-exam-score-calculator-in-java-15og | Technical tutorial | Article prose | `noopener noreferrer` | HTTP 200; no `noindex`; self-canonical | Exact keyword pending save | Source uses `USMLE score predictor` → `/`; owner must save the prepared edit before public verification. |
...
| R-universe | r-universe.dev | https://jiankn.r-universe.dev/nbmecalc | R package | Rendered README | expected none based on qualified representative page | Representative page HTTP 200; no `noindex`; no `X-Robots-Tag` | Prepared | Real dependency-free R package committed locally; exact `NBME score conversion` → `/nbme-score-conversion`; public GitHub repository and universe build pending. |
| Package Control | packagecontrol.io | https://packagecontrol.io/packages/NBMEcalc | Sublime Text package | Rendered README | none on qualified representative page | Representative page HTTP 200; no `noindex`; no `X-Robots-Tag` | Prepared | Working Sublime command and 4 passing tests committed locally; exact `USMLE Step score calculator` → `/`; public GitHub repository and channel PR pending. |
| Glama | glama.ai | Pending final server URL | Remote MCP directory | Rendered repository README | none on qualified representative page | Representative page HTTP 200; no `noindex`; canonical present | Prepared | Existing MCP repository now has `glama.json`, green CI, and exact `USMLE score predictor` → `/`; Glama submission pending. |
