...
| docs.rs | docs.rs | Not published | Rust library documentation | Homepage metadata | none on representative latest-version page | HTTP 200; no `noindex` on representative latest URL | Qualified / secret pending | `jiankn/nbmecalc-rust` v1.0.1 passed CI; crates.io publication requires repository secret `CARGO_REGISTRY_TOKEN` |
